...
func (_q *CommentQuery) QueryEntry() *EntryQuery {
...
func (_q *CommentQuery) QueryAuthor() *DoctorQuery {
...
func (_q *CommentQuery) WithEntry(opts ...func(*EntryQuery)) *CommentQuery {
	query := (&EntryClient{config: _q.config}).Query()
...
func (_q *CommentQuery) WithAuthor(opts ...func(*DoctorQuery)) *CommentQuery {
	query := (&DoctorClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := _q.withEntry; query != nil {
		if err := _q.loadEntry(ctx, query, nodes, nil,
			func(n *Comment, e *Entry) { n.Edges.Entry = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAuthor; query != nil {
		if err := _q.loadAuthor(ctx, query, nodes, nil,
			func(n *Comment, e *Doctor) { n.Edges.Author = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CommentQuery) loadEntry(ctx context.Context, query *EntryQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Entry)) error {
...
func (_q *CommentQuery) loadAuthor(ctx context.Context, query *DoctorQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Doctor)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Comment)
	for i := range nodes {
		fk := nodes[i].AuthorDoctorID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "author_doctor_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *CommentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withEntry != nil {
			_spec.Node.AddColumnOnce(comment.FieldEntryID)
		}
		if _q.withAuthor != nil {
			_spec.Node.AddColumnOnce(comment.FieldAuthorDoctorID)
		}
	}
